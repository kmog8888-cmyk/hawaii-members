...
export default function MenuPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] max-w-sm mx-auto pb-32">
      {/* ヘッダー */}
      <div className="px-5 pt-14 pb-5">
        <Link href="/me" className="flex items-center gap-1 text-xs text-[#ABABAB] hover:text-[#6B6B6B] mb-5">
      ...
      <div className="px-5 space-y-3">
        {filtered.length === 0 && (
          <div className="text-center py-16 text-[#CFCFCF]">
        ...
        {filtered.map((item) => {
          ...
          return (
            <div key={item.id} className="bg-[#F5F5F5] rounded-2xl border border-[#E8E8E8] overflow-hidden flex">
              {item.imageUrl && (
              ...
              <div className="flex-1 p-4 flex flex-col justify-between">
                ...
                <div className="flex items-center justify-end mt-3">
                  {qty === 0 ? (
                    ...
                    <div className="flex items-center gap-2 bg-[#F0F0F0] rounded-full px-1 py-1">
                      <button onClick={() => removeFromCart(item.id)} className="w-6 h-6 flex items-center justify-center text-[#6B6B6B] hover:text-[#1C1C1E]">
                        <Minus size={12} />
                      </button>
      ...
      {showCart && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative w-full max-w-sm mx-auto bg-[#F5F5F5] rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto">
            <div className="w-10 h-1 bg-[#E0E0E0] rounded-full mx-auto mb-5" />
            <h3 className="text-[#1C1C1E] font-semibold text-lg mb-4">注文内容</h3>
            ...
            <div className="border-t border-[#E8E8E8] pt-4 mb-4 space-y-3">
              {/* 受け取り時間 */}
              <div>
                <p className="text-[10px] text-[#ABABAB] tracking-widest uppercase mb-2">受け取り時間</p>
            ...
            <div className="flex items-center justify-between mb-4">
              <span className="text-[#6B6B6B] text-sm">合計</span>
              <span className="text-[#1C1C1E] font-bold text-xl">${cartTotal.toFixed(2)}</span>
            </div>
