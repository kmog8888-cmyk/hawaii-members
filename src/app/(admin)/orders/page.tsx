...
const STATUS_CONFIG: Record<string, { label: string; color: string; next: string | null; nextLabel: string | null }> = {
  ...
  ready:     { label: "受渡可",  color: "bg-[#BFE96A]/20 text-[#BFE96A]",  next: "completed", nextLabel: "渡し済" },
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {[["active", "対応中"], ["all", "すべて"], ["completed", "完了"]].map(([k, l]) => (
          <button key={k} onClick={() => setFilter(k)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filter === k ? "bg-[#BFE96A] text-[#0f0f0f]" : "bg-white/5 text-white/40 hover:bg-white/10"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        ...
        {filtered.map((order) => {
          ...
          return (
            <div key={order.id} className={`bg-[#1a1a1a] border rounded-2xl p-4 ${order.status === "ready" ? "border-[#BFE96A]/30" : "border-white/5"}`}>
              ...
              {st.next && (
                <button onClick={() => updateStatus(order.id, st.next!)}
                  className="w-full bg-[#BFE96A] text-[#0f0f0f] font-semibold py-2.5 rounded-xl text-sm hover:bg-[#d4f086] transition-colors">
                  {st.nextLabel}
                </button>
              )}
            </div>
